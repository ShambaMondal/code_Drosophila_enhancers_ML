...
#feature_matrix = "/home/Sam/Yoda_2TB/Sam_Work_Nencki/2016_Jan_Drosophila_project/analysis_on_the_rocks/InR_feature_matrix_long_seqs_4mer_3_mer_motifs_KcS2pills_04022016"
feature_matrix = "InR_data_matrix_regression"
...
def return_objects():
	...
	"""
	Y_objs = {
	'exp_S2_ctrl' : array([float(d[-8]) for d in data[1:]]),
	'exp_S2_dFOXO' : array([float(d[-7]) for d in data[1:]]),
	'exp_S2_EtOH' : array([float(d[-6]) for d in data[1:]]),
	'exp_S2_20E' : array([float(d[-5]) for d in data[1:]]),
	'exp_Kc_ctrl' : array([float(d[-4]) for d in data[1:]]),
	'exp_Kc_dFOXO' : array([float(d[-3]) for d in data[1:]]),
	'exp_Kc_EtOH' : array([float(d[-2]) for d in data[1:]]),
	'exp_Kc_20E' : array([float(d[-1]) for d in data[1:]])}
